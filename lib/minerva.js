...
class MinervaServer extends ApolloServerBase {
  ...
  getMiddleware (options = {}) {
    ...
    if (session) {
      middleware.push(sessionMiddleware(session))
    }
  ...
  installSubscriptionHandlers (server) {
    ...
    const {
      onDisconnect,
      onConnect: onConnectCustom = (connectionParams) => ({ ...connectionParams }),
      keepAlive,
      path,
    } = this.subscriptionServerOptions
    const { server: { session: sessionOptions } = {} } = this.options

    const onConnect = async (connectionParams, webSocket, context, ...rest) => {
      let session
      if (sessionOptions) {
        session = await new Promise((resolve) => {
          sessionMiddleware(sessionOptions)(context.request, {}, function () {
            resolve(context.request.session)
          })
        })
      }
      const customParams = await onConnectCustom(connectionParams, webSocket, context, ...rest)
      return { ...customParams, session }
    }
