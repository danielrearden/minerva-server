...
class BaseSchemaBuilder {
  ...
  build ({ models }, options = {}) {
    ...
    // Add resolvers for any scalars that were used
    Object.keys(scalars).forEach(scalarTypeName => {
      const scalar = schema.getType(scalars[scalarTypeName].name)
      if (scalar) {
        this.resolvers[scalar.name] = scalar
      }
    })
  ...
  generateFilterInput (model, possibleTypeName) {
    ...
    const filterableFields = Object.keys(model.rawAttributes).reduce((fields, attributeName) => {
      ...
      if (isFilterable && type && attribute.public !== false && attribute.filter !== false) {
        ...
        if (['JSONB', 'ARRAY', 'HSTORE'].includes(attribute.type.constructor.name)) {
        ...
        if (type === GraphQLIntRange) {
          fields[`${attributeName}_contains`] = { type: GraphQLInt }
          fields[`${attributeName}_contained`] = { type: GraphQLIntRange }
        } else if (type === GraphQLFloatRange) {
          fields[`${attributeName}_contains`] = { type: GraphQLFloat }
          fields[`${attributeName}_contained`] = { type: GraphQLFloatRange }
        } else if (type === GraphQLDateRange) {
          fields[`${attributeName}_contains`] = { type: GraphQLDate }
          fields[`${attributeName}_contained`] = { type: GraphQLDateRange }
        } else if (type === GraphQLDateTimeRange) {
          fields[`${attributeName}_contains`] = { type: GraphQLDateTime }
          fields[`${attributeName}_contained`] = { type: GraphQLDateTimeRange }
        }
  ...
  isFilterable (type) {
    switch (type.constructor.name) {
      case Sequelize.JSON.name:
      case Sequelize.JSONB.name:
      case Sequelize.HSTORE.name:
  ...
  isSortableAndFilterable (type) {
    switch (type.constructor.name) {
      ...
      case Sequelize.MACADDR.name:
      case Sequelize.INET.name:
      case Sequelize.CIDR.name:
      case Sequelize.CITEXT.name:
      case Sequelize.CHAR.name:
  ...
  getGraphQLOutputType (model, attributeName, typeString, type) {
    switch (typeString) {
      ...
      case Sequelize.RANGE.name: {
        return this.getRangeType(type._subtype)
      }
  ...
  getRangeType (typeString) {
    switch (typeString) {
      case Sequelize.SMALLINT.name:
      case Sequelize.MEDIUMINT.name:
      case Sequelize.INTEGER.name: return GraphQLIntRange
      case Sequelize.BIGINT.name:
      case Sequelize.REAL.name:
      case Sequelize.DOUBLE.name:
      case Sequelize.DECIMAL.name:
      case Sequelize.FLOAT.name: return GraphQLFloatRange
      case Sequelize.DATE.name: return GraphQLDateTimeRange
      case Sequelize.DATEONLY.name: return GraphQLDateRange
      default: return null
    }
  }
...
function resolveAssociationField (model, association) {
  return async function (instance, args, context, info) {
    ...
    const options = findOptionsFromInfo(association.target, info, {}, isPageType)
    return isPageType
