  ...
  describe('serialize', () => {
    ...
    it('should throw an error', () => {
      const assertSerializeThrows = ({ serialize }, lowerValue, upperValue) => {
        ...
        expect(() => serialize()).toThrow('should be an array')
        expect(() => serialize([lower])).toThrow('should be an array of length 2')
        expect(() => serialize([
    ...
    it('should throw an error', () => {
      const assertParseValueThrows = ({ parseValue }, lowerValue, upperValue) => {
        ...
        expect(() => parseValue([
          upper,
          lower,
        ])).toThrow('Lower bound cannot be higher than upper bound')
        expect(() => parseValue([
          { value: lower.value, inclusive: false },
          { value: lower.value, inclusive: false },
        ])).toThrow('Range cannot be empty')
        expect(() => parseValue([
          { value: lower.value, inclusive: true },
          { value: lower.value, inclusive: false },
        ])).toThrow('Range cannot be empty')
        expect(() => parseValue([
          { value: lower.value, inclusive: false },
      ...
      const date = new Date()
      const assertions = [
        [GraphQLDateRange, date.toISOString().substring(0, 10), new Date(date.getTime() + 86400000).toISOString().substring(0, 10)],
