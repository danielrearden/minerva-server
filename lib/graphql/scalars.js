...
function serializeRange (scalarName, valueScalar, result) {
  if (!Array.isArray(result)) {
    throw new TypeError(`${scalarName} should be an array.`)
  }
  // Empty set
  if (!result.length) {
    return []
...
function parseRangeValue (scalarName, valueScalar, value) {
  ...
  if (value[0].inclusive !== undefined && !_.isBoolean(value[0].inclusive)) {
    throw new TypeError('Lower bound inclusive field must be a boolean.')
  }
  if (value[0].value === undefined) {
    throw new TypeError('Lower bound must specify value field.')
  }
  if (!_.isPlainObject(value[1])) {
    throw new TypeError(`${scalarName} upper bound must be an object.`)
  }
  if (value[1].inclusive !== undefined && !_.isBoolean(value[1].inclusive)) {
    throw new TypeError('Upper bound inclusive field must be a boolean.')
  }
  if (value[1].value === undefined) {
  ...
  const bounds = value.map((bound, index) => ({
...
function validateBounds (parsedBounds) {
  const bounds = parsedBounds.map(({ value, inclusive }) => {
    return {
      value: (value && value.getTime) ? value.getTime() : value,
      inclusive,
    }
  })

  if (bounds[1].value !== null && bounds[0].value > bounds[1].value) {
    throw new TypeError('Lower bound cannot be higher than upper bound.')
  }
  ...
  if (
    ...
    bounds[0].value !== null &&
    bounds[1].value !== null &&
    (bounds[0].inclusive === false || bounds[1].inclusive === false)
  ) {
    throw new TypeError('Range cannot be empty.')
  }
}
